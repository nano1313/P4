...
DtPago CtrlReserva::pagoCredito(string nomFin){
    ...
    //Tarjeta *tar = t->encontrarInstancia(nomFin);   //GLOBAL A FUTURO
    //Credito* aux = dynamic_cast<Credito*>(&tar);
    //int des = aux->getDescuento();
...
int CtrlReserva::pagoDebito(string nomBanco){
    ...
    //Mtarjetas *t = getInstancia();
    //Debito *tar = t->encontrarInstancia(nomBanco) 
    //Debito* aux = dynamic_cast<Debito*>(&tar);    GLOBAL A FUTURO
    return total;
}
